...
# dataset=dataset[`ctDNA status at 4 weeks post surgery`=='Positive']
# dataset=dataset[`ctDNA status at 4 weeks post surgery`=='Negative']

# cox_fit <- coxph(Surv(DFS, DFS.event) ~ Sex + Age + location +`Stage I,II,III`+RAS+BRAF+MSI + `HER2 amplification` + `Histological type`, data = dataset)
dataset[,DFS:=DFS-70]
dataset=dataset[DFS>=0]
