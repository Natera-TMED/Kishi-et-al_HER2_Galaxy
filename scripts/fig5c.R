...
# load
# df <- read.csv("../cohort/copy_number_of_HER2_amp.csv")
df <- read.csv("/path/to/your/copy_number_of_HER2_amp.csv")

# RFS_Event
df$RFS_Event <- factor(df$RFS_Event, levels = c(0,1),
                       labels = c("No recurrence (N = 21)", "Recurrence (N = 8)"))
